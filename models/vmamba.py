...
class Backbone_VSSM(VSSM):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, norm_layer="ln",stages:Union[int,list[int]] = 3, **kwargs):
    ...
    def from_pretrained(cls,model_name,**kwargs):
        if 'vmamba-s' in model_name:
            vmamba_config_path = '/ssd1/zhuweiye/Multimodal-Mamba/configs/vmamba_s.json'
        elif 'vmamba-b' in model_name:
            vmamba_config_path = 'config_sys/'
        with open(vmamba_config_path, 'r') as f:
            vmamba_config = json.load(f)
        return cls(**vmamba_config)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            out, x = layer_forward(layer, x) # (B, H, W, C)
...
if __name__ == "__main__":
    # model = Backbone_VSSM(depths = [2, 2, 15, 2],dims = [128, 256, 512, 1024],norm_layer='ln',ssm_conv_bias=False,
    #                       mlp_ratio=4.0,patch_size=4,downsample_version='v3',forward_type="v4noz",ssm_d_state=1,patchembed_version='v2',
    #                       pretrained='/ssd1/zhuweiye/Multimodal-Mamba/pretrained_model/vision_encoder/vssm_base_0229_ckpt_epoch_237.pth'
    #                       )

    model = Backbone_VSSM(depths = [2, 2, 27, 2],dims = [96, 192, 384, 768],norm_layer='ln',ssm_conv_bias=True,
