...
class BackboneSiMBA(SiMBA):
    def __init__(self, in_chans=3, num_classes=1000, stem_hidden_dim=32, 
                 embed_dims=[64, 128, 320, 448], mlp_ratios=[8, 8, 4, 4], 
                 drop_path_rate=0, norm_layer=nn.LayerNorm, depths=[3, 4, 6, 3], 
                 sr_ratios=[4, 2, 1, 1], num_stages=4, token_label=True, 
                 pretrained=None, stages:Union[int,list[int]] = 3,**kwargs):
        super().__init__(in_chans, num_classes, stem_hidden_dim, embed_dims,
                          mlp_ratios, drop_path_rate, norm_layer, depths, sr_ratios,
                            num_stages, token_label, **kwargs)
...
if __name__ == "__main__":
    model = BackboneSiMBA(stem_hidden_dim = 64,
        embed_dims = [64, 128, 320, 512], 
        mlp_ratios = [8, 8, 4, 4], 
        norm_layer = partial(nn.LayerNorm, eps=1e-6), 
        depths = [3, 4, 12, 3], 
        sr_ratios = [4, 2, 1, 1], stages=3)
    #model = simba_b()
